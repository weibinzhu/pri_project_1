window.LCalendar = (function() {
    ...
    MobileCalendar.prototype = {
        init: function(params) { // 根据传入参数初始化日历
            this.type = params.type; // 将传入的type值赋值
            this.trigger = document.querySelector(params.trigger); // 传入挂载点
            if (this.trigger.getAttribute("data-lcalendar") != null) {  // 获取写在挂载点的参数
            ...
            if (params.minDate) { // 获取传入的minDate，maxDate参数
            ...
            this.bindEvent(this.type); // 启动bindEvent
        },
        bindEvent: function(type) {
            var _self = this;
            //呼出日期插件
            function popupDate(e) {
                _self.gearDate = document.createElement("div"); // 创建日期插件部件并添加到页面中
                ...
                dateCtrlInit(); // 调用插件定义的函数，初始化年月日插件默认值
                var lcalendar_cancel = _self.gearDate.querySelector(".lcalendar_cancel"); // 给各个部分绑定事件
            ...
            function dateCtrlInit() {
                ...
                if (/^\d{4}-\d{1,2}-\d{1,2}$/.test(_self.trigger.value)) { // 如果当前已经有值且符合格式，就用已经设置好的值
                ...
                _self.gearDate.querySelector(".date_yy").setAttribute("val", dateArr.yy); // 设置值
                _self.gearDate.querySelector(".date_mm").setAttribute("val", dateArr.mm);
                _self.gearDate.querySelector(".date_dd").setAttribute("val", dateArr.dd);
                setDateGearTooth(); // 重置日期节点个数
            }
            //呼出年月插件
            function popupYM(e) {
            ...
            function setDateGearTooth() {
                ...
                if (date_dd && date_dd.getAttribute("val")) {
                    ...
                    //返回月份的天数
                    var maxMonthDays = calcDays(yyVal, mmVal); // 计算日期范围
                    //p 当前节点前后需要展示的节点个数
            ...
            function closeMobileCalendar(e) {
                ...
                document.body.removeChild(_self.gearDate); // 整体移除
                _self.gearDate=null;
            }
